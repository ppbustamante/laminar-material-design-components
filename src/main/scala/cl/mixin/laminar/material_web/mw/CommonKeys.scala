...
/** Typical events / properties / etc. defined on Shoelace web components. We
  * selectively export them from this object into individual components that
  * define them.
  */
object CommonKeys extends CommonTypes {

  /** Emitted when the control’s "state" changes, similar to the browser's
    * `change` event.
    */
