package cl.mixin.laminar.material_web.mw
...
abstract class WebComponent(tagName: String) extends CommonTypes { this: Self =>
  ...
  /** Optional syntax for using built-in Laminar events:
    * `_.on(onDblClick.preventDefault) --> ...`
    */
  ...
  /** Instantiate this component using the specified modifiers.
    *
    * Modifiers can be the usual Laminar modifiers (e.g. onDblClick -->
    * observer), or they can be functions from this component to a modifier
    * (e.g. _.onClick --> observer)
    *
    * Scala 2 does not support union types, so use the [[of]] method instead.
    *
    * IntelliJ is unable to provide autocompletion when using this method, so I
    * suggest using [[of]] too. Upvote
  ...
  /** Instantiate this component with the specified modifiers.
    *
    * To pass standard Laminar modifiers with this method, use `_ => onDblClick
    * --> observer`.
    *
  ...
  /** Lets you set standard Laminar properties when using `of` method, e.g.
    * `sl.Button.of(_.L.attr := "value")`
    */
